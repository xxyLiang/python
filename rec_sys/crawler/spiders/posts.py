...
import scrapy
import pymysql
from crawler.items import TmjyItem
...
class PostsSpider(scrapy.Spider):
    ...
    db = pymysql.connect(host="localhost", user="root", password="651133439a", database="rec_sys", charset='utf8mb4')
    cursor = db.cursor()
    # 运行前注意是否是正式表
        ...
        if next_page is not None and re.match('http', next_page) is not None:
            ...
            if success_flag == 1:
                try:
                    ...
                    self.count += 1
                    if self.count % 50 == 0:
                        print("got posts from %d threads." % self.count)
                except:
                    self.db.rollback()
                    traceback.print_exc()
